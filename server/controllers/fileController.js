...
class FileController {
    ...
    async uploadFile(req, res) {
        try {
            ...
            let path = `${config.get('filePath')}/${user._id}/${parentFile ? parentFile.path : ''}/${fileName}`;
            ...
            while (fs.existsSync(path)) {
                ...
                path = `${config.get('filePath')}/${user._id}/${parentFile ? parentFile.path : ''}/${fileName}`;
                version++;
            }
        ...
        try {
            ...
            if (file.type === 'dir') {
                ...
                if (fs.existsSync(filePath)) {
                    fileService.deleteDir(file); // Вызов метода deleteDir для удаления директории
